...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardInfo = async () => {
      try {
        ...
        setTodos(todoRes.data.slice(0, 5)); // Just preview top 5
      } catch (error) {
  ...
  if (loading) {
     return <div className="animate-pulse bg-white/50 h-[80vh] rounded-xl flex items-center justify-center text-[var(--color-steelblue)]">Loading dashboard...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[var(--color-darkslate)]">Welcome back, {user?.name}! 👋</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <StatCard 
          ...
          value={stats.hackerrankProgress || 0} 
